...
func ToYaml(in interface{}) ([]byte, error) {
	jsonIterator := k8sJson.CaseSensitiveJsonIterator()
	jsonData, err := jsonIterator.Marshal(in)
	if err != nil {
		return nil, err
	}
...
func ParseYaml(in []byte, out interface{}) error {
	jsonBytes, err := yaml.YAMLToJSONStrict(in)
	if err != nil {
		return err
	}

	jsonIterator := k8sJson.CaseSensitiveJsonIterator()
	return jsonIterator.Unmarshal(jsonBytes, out)
}
